...
| G0.RC.1 certified residual constitution | ENFORCED_AT_CERTIFICATION_BOUNDARY | `ResidualDefinitionSpec` is a pre-evidence comparator definition, `ObservedDifference` is an observation result, and `CertifiedResidual` is a separate content-bound contract. Certification requires an exact frozen parent reference, an authority-issued `AuthenticatedObservationBinding`, scope, pre-registered reconstruction and comparator specifications, a non-empty reconstructible difference witness, and a `Trace`. A matching comparator must report an actual difference; a description, residual id, legacy `Residual`, or caller-created/post-hoc specification cannot certify one. Residual typing, structural reopen authority, and expansion remain deferred to G0.RT.1 and G0.SE.1. |
