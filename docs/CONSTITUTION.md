...
| `LineageEdgeBindsExactReopenSpecification` | PROVED (at contract level) | When supplied, a `ProofLineageEdge` binds its reopen identity to the exact embedded `ReopenExperimentSpecification`, and its parent must be declared by that specification. |
