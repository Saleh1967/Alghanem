...
| G0.RC.1 certified residual constitution | DECLARED_DEFERRED_CONTRACT_ONLY | `ResidualCertificationCandidate` composes the existing frozen `ResidualDefinitionSpec` and its `BirthSemanticsContentIdentity`, but it is not an authority-issued certification. `FrozenFactorRef`/`BornBridgeRef` remain caller-constructible contract references until G0.FA.1; reconstruction-attempt and comparison authorities are also absent. `ResidualDefinitionSpec != ObservedDifference != CertifiedResidual != LegacyResidual` remains a declared distinction only. G0.RC.1 must not be marked enforced until an authority-issued frozen parent, authorized reconstruction attempt, and authority-issued comparison result exist. Residual typing and structural expansion remain deferred to G0.RT.1 and G0.SE.1. |
