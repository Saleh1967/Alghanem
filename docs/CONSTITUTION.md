...
| Verified invariant preservation | ENFORCED_AT_INVARIANT_GATE | Structural admission issues an opaque `admission_id` plus a deterministic, content-based `transition_fingerprint`; verification provenance binds that exact transition identity and the sealed registry's `registry_snapshot_id` and content-based `registry_content_hash`, in addition to claim, anchors, and trace: `OccurrenceIdentity != ContentIdentity`, and both are recorded together, not one instead of the other. `InvariantVerificationGate.assess_all_preserved` and `InvariantVerificationBundle` require exactly one successful verification for every declared preserved component; both `InvariantVerificationDecision` and `InvariantVerificationBundle` are gate-issued only (`VerificationDecision must be gate-issued`), so a caller cannot hand-build a `VERIFIED` decision and a later layer that trusts `decision.status == VERIFIED` cannot be fooled by a self-declared result. `DEFER` is a real, distinct status: epistemic non-answers (unregistered extractor, failing extractor, ambiguous non-`bool` comparison) are `DEFER`, a disproved invariant is `BLOCK`, and internal/programming errors are caught into neither -- they propagate. Failures remain in an auditable `InvariantVerificationDecision`, and the convenience API raises a typed error carrying that decision. Registration authority is still distinct from snapshot resolution authority, but snapshot identity is not an authority license. Verification is not yet wired into `StructuralAdmissionGate` or evidential sufficiency; `No epistemic promotion` remains `DECLARED_DEFERRED`. |
