...
| `NamedCriticalPairs != GeneralizedLaw` | ENFORCED_AT_INTERVENTION_FOOTPRINT | The product of the footprint module is an enumerated list of named pairs with their verdicts and reasons (`overlap`, `shift`, `out_of_range`) over named coordinate configurations, not a generalized confluence or commutation law over the whole intervention algebra. `reference_matrix` covers the five intervention types pairwise across seven declared configurations on a distinct-atom sequence, excluding identical interventions by declaration; its scope and its limits are recorded in `DERIVED_LAW_SCOPE_NOTE`, `IDENTICAL_PAIR_EXCLUSION_NOTE`, and `VALUE_COINCIDENCE_NOTE`. Because the model reads coordinates and not values, agreement produced by repeated atoms is value coincidence, not structural commutation, and is demonstrated by an explicit test instead of absorbed by widening the definition. Nothing beyond the measured configurations is claimed. |
