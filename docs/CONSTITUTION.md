...
| G0.C.1 minimal claim constitution | ENFORCED_AT_CONTENT_ENCODER | `ClaimCandidate` is a reviewable occurrence of structured `ClaimContentManifest` (`anchor`, `predicate`, `polarity`, `scope`, optional typed qualifications), not truth, evidence, knowledge, or a license. `CanonicalClaimContentEncoder` alone issues a `ClaimContentIdentity` from canonical structured content. `ClaimOccurrenceRef != ClaimContentIdentity`: separate occurrences may share canonical content. Scope and polarity are encoded content, so neither scope changes nor affirmation/negation can share a content identity. Text is not a claim input or identity; rendering is deferred. No evidence or evidence-binding field exists in `ClaimCandidate`; claim-specific evidence binding is deferred to G0.EB.1. |
