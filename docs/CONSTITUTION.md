...
| Verified invariant preservation | PARTIALLY_ENFORCED | `InvariantVerificationGate.verify` independently checks a single named invariant against an already-admitted `StructurallyAdmissibleTransition`, producing an `InvariantVerification` (`preserved: bool`) from an `InvariantObservation` of the actual before/after values. The candidate never supplies the extractor: an `InvariantSpec` only names a `component` and a registered `extractor_id`; resolving that id to a callable is the sole authority of an `InvariantExtractorRegistry`, so a candidate cannot embed `lambda before, after: True` and verify itself. `ClaimedInvariant != VerifiedInvariant`: declaring a spec, or a name in `preserved`, is still not proof. Verification is not yet wired into `StructuralAdmissionGate` or required for admission, and it checks one declared invariant at a time, not the full set. |
