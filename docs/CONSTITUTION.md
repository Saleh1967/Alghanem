...
| Canonical content identity | ENFORCED_AT_CONTENT_IDENTITY_BOUNDARY | `OccurrenceIdentity != ContentIdentity`. `admission_id` identifies one admitted occurrence. `TransitionContentIdentity` (issued only by `CanonicalTransitionEncoder.encode`, never by the transition itself) identifies the canonicalized structural content of an admitted transition under an explicitly versioned canonicalization contract (`algorithm`, `canonicalization_version`, `digest` -- never a bare hash string). Content identity excludes occurrence-only identifiers (`admission_id` is not part of `CanonicalTransitionManifest`) and cannot be issued for payloads outside the canonicalization contract: `CanonicalizationError` is raised, not a best-effort `repr()` fallback (`StableRepr != CanonicalIdentity`). `Canonicalizable != True` -- successful canonicalization is not a claim that the represented content is semantically meaningful, licensed, or verified, and `Canonicalization != Certification`. `CanonicalStateManifest.from_payload` snapshots opaque payloads into an immutable tuple tree at construction time, so mutating the original payload afterward does not change an already-issued manifest or content id (`SnapshotIdentity is stable after issuance`). `preserved`/`changed` are canonicalized as sorted, deduplicated sets (membership, not declared order); every other covered sequence (before/after state, evidence, trace, residuals) preserves declared order (`CanonicalizationMustPreserveDeclaredStructure`, not `CanonicalizationMustEraseOrder`). Two transitions sharing a `TransitionContentIdentity` are known to share canonicalized structural content only -- not the same epistemic authority, verification status, or run (`SameContentID != SameEpistemicAuthority`). Extractor/registry content identity, and wiring content identity into `InvariantVerificationProvenance`, are explicitly deferred to a dedicated follow-up. |
