...
| G0.BV.1 authority-issued birth verdict | ENFORCED_AT_VERDICT_AUTHORITY | `BirthAssessment` records only `BIRTH`, `NO_BIRTH`, or `DEFER`, with its trace, residuals, reason, and a derived scope containing experiment/content/evidence/evaluator/weaker-model provenance. `BirthVerdictAuthority` is the sole issuer of the corresponding scoped `BirthVerdictStatus` values. A verdict is not a `Freeze`, and this authority issues no frozen factor or residual certification. |
...
freeze authority. G0.BV.1 is the later, narrow authority boundary that may
issue a scoped verdict once an assessment and its evaluator provenance are
present; it does not issue a freeze.
