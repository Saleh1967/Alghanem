...
**وبطلانُ الإدخال ليس حجبًا** (`InvalidInputIsNotABlock`): وثيقةٌ مفتاحُها
مجهولٌ، أو حقلُها المطلوبُ غائب، أو نوعُها غيرُ قانونيّ، أو بصمتُها المُعلَنة
تخالف بصمةَ محتواها المُعاد حسابُه — كلُّ ذلك بطلانٌ في أصل التكوين: لم تقم
قضيّةٌ صالحةٌ لأن تُحكَم. وهو غيرُ الفساد؛ فالفسادُ أن يقوم الشيءُ ثمّ يختلّ
وجهُ عمله، والحجبُ من هذا البابِ لأنّه مخالفةٌ ثبتت بعد أن أصبح الحكمُ ممكنًا.
ولذلك لا يحمل هذا النوعُ حقلَ `ExecutionOutcome` أصلًا: الخلطُ غيرُ قابلٍ للقول
لا مرفوضٌ بعد وقوعه.

    InputInvalidity  =  بطلانُ التكوين      BLOCK  =  اختلالٌ بعد القيام
...
INVALID_INPUT_IS_NOT_A_BLOCK: Final[str] = (
    "بطلانُ الإدخال ليس حجبًا: الأوّلُ بطلانٌ في أصل التكوين فلم تقم قضيّةٌ "
    "صالحةٌ لأن تُحكَم، والثاني مخالفةٌ ثبتت بعد أن أصبح الحكمُ ممكنًا؛ ومن "
    "جمعهما أصدر حكمًا على ما لم يستطع أن يقرأه"
)

INPUT_CONSTITUTION_FAILURE: Final[str] = (
    "بطلانُ تكوين القضيّة: البطلانُ أن يفوت أصلُ التكوين فلا يقوم الشيء، "
    "والفسادُ أن يقوم الشيءُ ثمّ يختلّ وجهُ عمله؛ و`INVALID_INPUT` من الأوّل "
    "لا من الثاني"
)
...
class InputFaultKind(Enum):
    """أجناسُ بطلان التكوين؛ مفردةٌ مغلقةٌ، ولا واحدَ منها يُقرَأ حكمًا."""
...
class InputValidation:
    ...
    def __post_init__(self) -> None:
        ...
        if (self.standing is InputStanding.INVALID) != bool(self.faults):
            raise TypeError(
                "البطلانُ يُسمّي عيوبَه، والصحّةُ لا عيبَ فيها؛ و"
                + INVALID_INPUT_IS_NOT_A_BLOCK
            )
