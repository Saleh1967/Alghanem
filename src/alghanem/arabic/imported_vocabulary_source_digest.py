...
* وهذا يعني بلا تلطيف: `facf6720…ba85f` بعد هذه الوحدة **مُعلَنٌ لا مُعادَ
  اشتقاقه**، كما كان قبلها؛ والمتغيِّر قابليةُ اختباره لا حالُ دليله. ولذلك
  بقيت `SOURCE_DIGEST_REDERIVATION_NOTE` في الوحدة الأمّ بصيغتها التحذيرية
  نفسها غير مُبدَّلة، فتبديلُها بصيغةٍ حاسمة يجعل النصّ يشهد بما لم يجرِ.
* ولا مستهلكَ يفترض صحّتها: لا وحدةَ نواة ولا بوّابة ولا وحدةَ عربية تستدعي
  شيئًا من هنا، فلو فشلت المطابقة عند إيداع الحمولة لم يُبنَ على الافتراض
  الضمنيّ شيءٌ يُتتبَّع. وهذا مفحوصٌ باختبارٍ يمسح الشجرة لا مُصرَّحٌ به فقط.
...
SOURCE_DIGEST_REMAINS_DECLARED_NOTE: Final = (
    "الرقم facf6720…ba85f بعد إيداع المخطَّط مُعلَنٌ لا مُعادَ اشتقاقه، كما "
    "كان قبله سواءً بسواء: المتغيِّر قابليةُ اختباره لا حالُ دليله. وشاهد "
    "ذلك في المستودع لا في هذه الجملة: اختبار الحمولة المُودَعة يبقى skipped "
    "لا passed حتى تُودَع الحمولة، فجاهزيةُ الآلة ليست دليلًا، وقراءتها "
    "دليلًا هي عينُ الخطأ الذي تحرسه هذه الوحدة"
)

NO_CONSUMER_ASSUMES_VERIFIED_DIGEST_NOTE: Final = (
    "لا شيء في المسار الحالي يعتمد على صحّة هذه البصمة: لا وحدةَ نواة ولا "
    "بوّابة ولا وحدةَ عربية تستدعي دوالّ هذا الملف، وتصديره من alghanem.arabic "
    "إتاحةٌ لا استهلاك. فلو فشلت المطابقة حين تُودَع الحمولة، لا يوجد ما بُني "
    "على افتراضٍ ضمنيّ بصحّتها يلزم تتبُّعه — وهذا الغياب مفحوصٌ باختبارٍ يمسح "
    "الشجرة، لا مُصرَّحٌ به هنا وحسب"
)
