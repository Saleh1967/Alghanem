...
الرابعة لغيابٍ مُسمًّى (`APPLICATION_STOPS_AT_THE_FOURTH_LINK_NOTE`). فالفرقُ
...
APPLICATION_STOPS_AT_THE_FOURTH_LINK_NOTE: Final[str] = (
    ...
    "الناس:٢ على الحلقات ٤–١٣ فعلًا، فوقفت عند **الحلقة الرابعة (الوضع بالنقل)** "
    "لغيابٍ مُسمًّى: البطاقةُ لا تُعلن طريقَ معرفة الوضع من `TransmissionStanding`، "
    "وهو عينُ ما تفتقده الحلقةُ الثانية عشرة. فالقيدُ (ب) مُصرَّحٌ غيرُ مُرمَّزٍ "
    "لا لفراغٍ نظريّ كالقيد (أ)، بل لأنّ التطبيق الأوّل انقطع عند موضعٍ مُسمًّى؛ "
    "ووحدةٌ تقيس تطبيقًا لم يكتمل هي بعينها «وصفُ مرحلةٍ أخرى من آلة القياس» "
    "الذي يمنعه هذا القيدُ نفسه."
)
