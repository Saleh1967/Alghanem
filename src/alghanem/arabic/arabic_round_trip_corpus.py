...
FATIHA_ROUND_TRIP: Final[RoundTripCorpusMeasurement] = RoundTripCorpusMeasurement(
    ...
    table_digest="c34c24d5f8a0905822ce2bd87dcf745524d8f3667cba07a0b05808907569c56d",
)
"""تسعٌ وعشرون كلمةً من إيداع الفاتحة، وموضعُ التوقّف واحدٌ مُسمًّى.

بعد أن عُمِّم الحيادُ على **مفتتح الكلمة قبل أوّل حركةٍ مكتوبة** — لا على
الألف وحدَه — لم يبقَ في هذا الإيداع إلّا وقوفٌ واحدٌ عند
`SEGMENTATION_TWO_ADJACENT_SAKINS` بعد أن كانت ثمانيَ كلمات. والسبعُ الذاهبةُ
كلُّها لامُ تعريفٍ مكتوبةٌ قبل مشدَّد، وحالةُ تلك اللام غيرُ مكتوبةٍ أصلًا
فلا تُحاسَب حسابَ الساكن المكتوب. والواقعةُ الباقيةُ سببُها آخرُ مُسمًّى: مدٌّ
قبل مشدَّدٍ في وسط الكلمة (`الضَّالِّينَ`)، وهو خارجُ المفتتح بتمامه.

وهذه هي النتيجةُ التي تُقال ولا تُجمَّل: **الاسترجاعُ لم يرتفع**. بقي عند ستَّ
عشرةَ كلمة، وانتقل السبعُ من رفضٍ في طبقة المقطع إلى **اختلافِ ترتيبٍ** في أعلى
الخطّ — فصارت المُخالِفاتُ اثنتَي عشرةَ بعد خمس، كلُّها بفقدٍ صفرٍ وزيادةٍ صفر،
سببُها كتابةُ المرماز الشدّةَ قبل الحركة وتسويةُ `NFC` بعدها. فلامُ التعريف لم
تكن يومًا مانعَ الاسترجاع؛ المانعُ ترتيبُ العلامات، وقد صار الآن مكشوفًا
معدودًا بعد أن كان مستورًا خلف رفضٍ أسبق.
