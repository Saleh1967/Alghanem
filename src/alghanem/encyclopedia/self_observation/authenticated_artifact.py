...
class AuthenticatedRepositoryArtifact:
    ...
    def __init__(self, snapshot, artifact_path, blob_sha, observation_run, *, _token=None):
        if _token is not _AUTHORITY_TOKEN:
            raise SelfObservationContractError(
                "authenticated artifacts may only be issued by the observation authority"
            )
        if not isinstance(snapshot, AuthenticatedRepositorySnapshot):
            raise SelfObservationContractError("authenticated artifact requires an authenticated snapshot")
        ...
        if observation_run != snapshot.observation_run:
            raise SelfObservationContractError("artifact and snapshot must share an observation run")
