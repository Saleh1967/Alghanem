...
class AuthenticatedRepositoryTransition:
    ...
    def __init__(self, from_snapshot, to_snapshot, observation_run, *, _token=None):
        if _token is not _AUTHORITY_TOKEN:
            raise SelfObservationContractError(
                "authenticated transitions may only be issued by the observation authority"
            )
        if not isinstance(from_snapshot, AuthenticatedRepositorySnapshot) or not isinstance(
            to_snapshot, AuthenticatedRepositorySnapshot
        ):
            raise SelfObservationContractError("transition requires authenticated snapshots")
        if from_snapshot.snapshot.repository_identity != to_snapshot.snapshot.repository_identity:
            raise SelfObservationContractError("transition snapshots must share a repository")
        if from_snapshot.snapshot.commit_sha == to_snapshot.snapshot.commit_sha:
            raise SelfObservationContractError("transition snapshots must have distinct commits")
        if from_snapshot.observation_run != observation_run or to_snapshot.observation_run != observation_run:
            raise SelfObservationContractError("transition snapshots must share the observation run")
