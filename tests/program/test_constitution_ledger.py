...
def test_the_repository_document_is_read_without_refusal() -> None:
    ...
    assert ledger.tables.table_count == 21
    assert len(ledger.tables.law_tables) == 20
    ...
    assert (
        ledger.laws.status_counts[DeclaredLawStatus.ENFORCED_AT_EXPERIMENTAL_AUTHORITY]
        > 0
    )
    assert (
        ledger.laws.status_counts[
            DeclaredLawStatus.ENFORCED_AT_EXPERIMENTAL_EVIDENCE_GATE
        ]
        > 0
    )
